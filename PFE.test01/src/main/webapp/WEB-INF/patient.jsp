						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  ...
							  <tr> 
							      ...
							      <th>Reartbit Reference</th>
							      <th>Sweat Reference</th>
						  ...
						  <% int j=0; %>
						  <c:forEach var="u" items="${patientList}">
          					
          					<% j=j+1; %>
        
							<tr>
							    <td id="<%= "wSNumber"+ j %>">${u.user.username}</td>
								<td id="<%= "hVersion"+ j %>">${u.hbRef}</td>
								<td id="<%= "sVersion"+ j %>" class="center">${u.SRef}</td>
								<td id="<%= "cType"+ j %>" class="center">${u.watch.watchSerialNumber}</td>
								...
								<td class="center">
									...
									<a class="btn btn-danger" href="deletPatient?id=${u.id}">
...
<div class="modal hide fade" id="myModal">
			...
			<form:form method="POST" id="frmew" commandName="patientModel" onsubmit="return validation1()">
   <table>
   
 
    <tr>
        <td><form:label path="user.id" style="display: none;">user UserName</form:label></td>
        <td><form:input path="user.id" id="wsnb"  style="display: none;"/></td>
    </tr>
    <tr>
        <td><form:label path="hbRef">Heart Beat Reference</form:label></td>
        <td><form:input path="hbRef" id="hv"/></td>
    </tr>
    <tr>
        <td><form:label path="SRef">Sweat Reference</form:label></td>
        <td><form:input path="SRef" id="sv"/></td>
    </tr>
    <tr>
        <td><form:label path="watch.id" style="display: none;">watch Serial Number</form:label></td>
        <td><form:input path="watch.id" id="ct" style="display: none;"/></td>
    </tr>
...
<div class="row-fluid sortable">	
				<div class="box span12">
					<div class="box-header">
						<h2><i class="icon-align-justify"></i><span class="break"></span>Patien-Watch</h2>
						<div class="box-icon">
					...
					<div class="box-content">
					<form:form method="POST"  id="adui" action="addPatient" commandName="patientModel" onsubmit="return validation()">
   <table>
<!--    <div align="center"> -->
     
 
    <tr>
        <td><form:label path="user.id">user userName</form:label></td>
         <td>
        <form:select data-rel="chosen" class="control-label input-xlarge focused" for="selectError" path="user.id" id="userId" items="${patientAv}" />
...
<%--         <td><form:input class="input-xlarge focused" path="user.id" id="user.id"/></td>    --%>
    ...
    <tr>
        <td><form:label path="watch.id">watch Serial Number</form:label></td>
      
        <td>
<%--         <form:input class="input-xlarge focused" path="watch.id" id="watch.id"/> --%>
           <form:select data-rel="chosen" class="control-label input-xlarge focused" for="selectError" path="watch.id" id="watch.id" items="${WatchAv}" />
        </td>
    ...
    <tr>
        <td><form:label path="hbRef">Heart Beat Reference</form:label></td>
        <td><form:input class="input-xlarge focused" path="hbRef" id="hbRef"/></td> 
                   
    </tr>
   
    <tr>
        <td><form:label path="SRef">Sweat Reference</form:label></td>
        <td><form:input class="input-xlarge focused" path="SRef" id="SRef"/></td>
